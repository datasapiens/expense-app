...
export const incomeAndCategoryData = (
  ...
  if (transactions.length > 1) {
    ...
    result.forEach((label) => {
      if (label.type === "income") incomeLabels.push(label.label);
    });
    const income: any[] = [];
...
export const expensesAndCategoryData = (
  transactions: Transaction[],
  categories: Category[]
) => {
  ...
  if (transactions.length > 1) {
    ...
    const labels: any[] = [];
    result.forEach((label) => {
      if (label.type === "expenses") labels.push(label.label);
    });
